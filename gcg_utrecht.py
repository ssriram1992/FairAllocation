...
def basic_model():
    ...
    for i in range(num_rounds):
        for j in range(n):
            # =============>> GCG: These 4 constraints have been reformulated.
            if j in bases:
                M.addConstr(x[i, bases.index(j)]+gp.quicksum(x[i, k] * adj[bases[k], j] for k in range(len(bases))) + bigM*(1-tau[i, j]) >= sufficient[j] )
                M.addConstr(x[i, bases.index(j)]+gp.quicksum(x[i, k] * adj[bases[k], j] for k in range(len(bases)))  <= sufficient[j]-1 + bigM*tau[i,j])
            else:
                M.addConstr(gp.quicksum(x[i, k] * adj[bases[k], j] for k in range(len(bases))) + bigM*(1-tau[i, j]) >= sufficient[j])
                M.addConstr(gp.quicksum(x[i, k] * adj[bases[k], j] for k in range(len(bases))) <= sufficient[j]-1 + bigM*tau[i,j])
...
# =============>> GCG: These 2 constraints have been reformulated. (Note: this may not work for other objectives than 'maxmin')
for i in range(n):
    mainM.addConstr(tau_max >= tau_col[i])
    mainM.addConstr(tau_min <= tau_col[i])
...
if objective == 'efficiency':
    ...
    # =============>> GCG: This may not work correctly.
...
(goto-char 0)
(replace-regexp "\\[\\([0-9]+\\)\\]" "#\\1")) ;; To make the function interactive, put the cursor at the end of the last parenthesis and press C-x C-e. Then switch to the ambulances.lp buffer, and do M-x gcg-lp
"""
